...
import {
  ...
  reducer,
} from '../../../../src/features/recipe/redux/fetchCategories';

const middlewares = [thunk];
const mockStore = configureMockStore(middlewares);

describe('recipe/redux/fetchCategories', () => {
  afterEach(() => {
    nock.cleanAll();
  });
  ...
  it('dispatches success action when fetchCategories succeeds', () => {
    ...
    return store.dispatch(fetchCategories())
      .then(() => {
        ...
        expect(actions[0]).toHaveProperty('type', RECIPE_FETCH_CATEGORIES_BEGIN);
        expect(actions[1]).toHaveProperty('type', RECIPE_FETCH_CATEGORIES_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when fetchCategories fails', () => {
    ...
    return store.dispatch(fetchCategories({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).toHaveProperty('type', RECIPE_FETCH_CATEGORIES_BEGIN);
        expect(actions[1]).toHaveProperty('type', RECIPE_FETCH_CATEGORIES_FAILURE);
  ...
  it('handles action type RECIPE_FETCH_CATEGORIES_BEGIN correctly', () => {
  ...
  it('handles action type RECIPE_FETCH_CATEGORIES_SUCCESS correctly', () => {
    const prevState = { fetchCategoriesPending: true };
    const state = reducer(
      prevState,
      { type: RECIPE_FETCH_CATEGORIES_SUCCESS, data: {} }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.fetchCategoriesPending).toBe(false);
  });

  it('handles action type RECIPE_FETCH_CATEGORIES_FAILURE correctly', () => {
    const prevState = { fetchCategoriesPending: true };
    const state = reducer(
      prevState,
      { type: RECIPE_FETCH_CATEGORIES_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.fetchCategoriesPending).toBe(false);
  ...
  it('handles action type RECIPE_FETCH_CATEGORIES_DISMISS_ERROR correctly', () => {
